...
## What it does
Engram turns engineering events into governed memories, retrieves valid knowledge across sessions, explains every selected memory, preserves supersession history, and consolidates duplicates without deleting evidence.
...
## CockroachDB tools
Distributed Vector Indexing performs project-scoped semantic candidate search. CockroachDB Cloud Managed MCP Server gives developer tools read-only schema, trace, and cluster inspection.
